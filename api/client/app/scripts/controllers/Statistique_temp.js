...
angular.module('scolariteApp')
  .controller('StatistiqueCtrl', function (API_URL, $scope, $http) {
    ...
    {
      $http.get(API_URL + '/etudiant').then(function(data){
      $scope.resultats=data.data;
         for(var j=0;j<$scope.departements.length;j++)
            {
              if($scope.departements.nomDepartement=="Tous les departements")
              {
                departement=$scope.departements.id;
              }
            }
        if($scope.affiche.departement==false)
        {     
          for(var j=0;j<$scope.resultats.length;j++)
            {
              if($scope.resultats[j].departementId==$scope.stat.departement)
              {
        ...
        else
        {
          for(var i=0;i<$scope.resultats.length;i++)
            {
              for(var k=0;k<$scope.departements.length;k++)
                {
                  if($scope.resultats[i].departementId==$scope.departements[k].id)
                  {
                    $scope.resultats[i].departement=$scope.departements[k].nomDepartement;
                  }
                }
            }
        }
        resultats=[];
        if($scope.affiche.option==false)
        {     
          for(var j=0;j<$scope.resultats.length;j++)
            {
              if($scope.resultats[j].optionId==$scope.stat.option)
              {
                resultats.push($scope.resultats[j]);
              }
            }
          ...
          for(var i=0;i<$scope.resultats.length;i++)
            {
              for(var k=0;k<$scope.options.length;k++)
                {
                  if($scope.resultats[i].optionId==$scope.options[k].id)
                  {
                    $scope.resultats[i].option=$scope.options[k].nomOption;
                  }
                }
            }
        }
        resultats=[];
        if($scope.affiche.formation==false)
        {     
          for(var j=0;j<$scope.resultats.length;j++)
            {
              if($scope.resultats[j].formationId==$scope.stat.formation)
              {
                resultats.push($scope.resultats[j]);
              }
            }
        ...
        else
        {
          for(var i=0;i<$scope.resultats.length;i++)
            {
              for(var k=0;k<$scope.formations.length;k++)
                {
                  if($scope.resultats[i].formationId==$scope.formations[k].id)
                  {
                    $scope.resultats[i].formation=$scope.formations[k].nomFormation;
                  }
                }
            }
        }
        resultats=[];
        if($scope.affiche.niveau==false)
        {     
          for(var j=0;j<$scope.resultats.length;j++)
            {
              if($scope.resultats[j].niveauId==$scope.stat.niveau)
              {
                resultats.push($scope.resultats[j]);
              }
            }
        ...
        else
        {
          for(var i=0;i<$scope.resultats.length;i++)
            {
              for(var k=0;k<$scope.niveaux.length;k++)
                {
                  if($scope.resultats[i].niveauId==$scope.niveaux[k].id)
                  {
                    $scope.resultats[i].niveau=$scope.niveaux[k].nomNiveau;
                  }
                }
            }
        }
    ...
    $scope.selectNiveau=function(){
        if ($scope.stat.niveau=="58816b43cfea990c082a133a")
            $scope.affiche.niveau=true;
        else
             $scope.affiche.niveau=false;
        }
    $scope.selectOption=function(){
        if ($scope.stat.option=="58816c6c06505332096c3fe1")
            $scope.affiche.option=true;
        else
             $scope.affiche.option=false;
        }
    $scope.selectFormation=function(){
        if ($scope.stat.formation=="58816b0ecfea990c082a1339")
            $scope.affiche.formation=true;
        else
             $scope.affiche.formation=false;
        }

    $scope.selectSexe=function(){
        if ($scope.stat.sexe=="Toutes sexes")
            $scope.affiche.sexe=true;
        else
             $scope.affiche.sexe=false;
        }
    $scope.selectNationalite=function(){
        if ($scope.stat.nationalite=="Toutes nationalite")
            $scope.affiche.nationalite=true;
        else
             $scope.affiche.nationalite=false;
        }
    $scope.selectBac=function(){
        if ($scope.stat.bac=="Toutes series")
            $scope.affiche.bac=true;
        else
             $scope.affiche.bac=false;
        }

    $scope.selectDepartement=function(){
        if ($scope.stat.departement=="58816b0ecfea990c082a1339")
            $scope.affiche.departement=true;
        else
